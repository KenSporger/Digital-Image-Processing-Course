...
Point mousePositionSample= mousePosition;
...
//green for predict, blue for turth
int main()
{
    RNG rng;
	...
	while (1)
	{
        ...
        if (++cnt == 1)
        {
            ...
            cnt = 0;
            if (KALMAN)
                mousePositionSample = predict_pt;
            else
                mousePositionSample = mousePosition;
        }

        circle(image,mousePositionSample,5,Scalar(0,255,0),3);    //predicted point with green
		imshow("kalman", image);
        waitKey(1);
